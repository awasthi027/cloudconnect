...
*----------------------------------------------------------------*
*
* MODULE SUMMARY : Form HTTP headers from key value and dict     *
*
*
*
*
...
extension HTTPHeader {
    ...
    ///
    /// CloudService offers a default Accept-Language header that accumulates and encodes the system's preferred languages.
    ...
    ///
    /// CloudService offers a default accept encoding value that provides the most common values. Use
    ...
    ///
    /// CloudService provides built-in methods to produce `Authorization` headers. For a Basic `Authorization` header use
    ...
    ///
    /// All CloudService `ParameterEncoding`s and `ParameterEncoder`s set the `Content-Type` of the request, so it may not be necessary to manually
    /// set this value.
    ///
...
public extension HTTPHeaders {
    /// The default set of `HTTPHeaders` used by CloudService. Includes `Accept-Encoding`, `Accept-Language`, and
    /// `User-Agent`.
...
extension HTTPHeader {
    /// Returns CloudService's default `Accept-Encoding` header, appropriate for the encodings supported by particular OS
    /// versions.
    ///
    ...
    /// Returns CloudService's default `Accept-Language` header, generated by querying `Locale` for the user's
    /// `preferredLanguages`.
    ///
    ...
    /// Returns CloudService's default `User-Agent` header.
    ///
    /// See the [User-Agent header documentation](https://tools.ietf.org/html/rfc7231#section-5.5.3).
    ///
    /// Example: `iOS Example/1.0 (org.CloudService.iOS-Example; build:1; iOS 13.0.0) CloudService/5.0.0`
    public static let defaultUserAgent: HTTPHeader = {
        ...
        let cloudService = "CloudService/\(libVersion)"

        let userAgent = "\(executable)/\(appVersion) (\(bundle); build:\(appBuild); \(osNameVersion)) \(cloudService)"
