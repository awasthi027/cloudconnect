...
*----------------------------------------------------------------*
*
* MODULE SUMMARY : Multipart request form                        *
*
*
*
*
...
open class MultipartFormData {
    ...
    struct BoundaryGenerator {
        ...
        static func randomBoundary() -> String {
            ...
            return String(format: "cloudConnect.boundary.%08x%08x", first, second)
        }
    ...
    private var bodyPartError: CSError?
    ...
    public func append(_ fileURL: URL, withName name: String, fileName: String, mimeType: String) {
        ...
        do {
            ...
            guard isReachable else {
                setBodyPartError(withReason: .bodyPartFileNotReachable(url: fileURL))
                return
            }
        } catch {
            setBodyPartError(withReason: .bodyPartFileNotReachableWithError(url: fileURL))
            return
        }
        ...
        guard fileManager.fileExists(atPath: path, isDirectory: &isDirectory) && !isDirectory.boolValue else {
            setBodyPartError(withReason: .bodyPartFileIsDirectory(url: fileURL))
            return
        }
        ...
        do {
            guard let fileSize = try fileManager.attributesOfItem(atPath: path)[.size] as? NSNumber else {
                setBodyPartError(withReason: .bodyPartFileSizeNotAvailable(url: fileURL))
                return
            }

            bodyContentLength = fileSize.uint64Value
        } catch {
            setBodyPartError(withReason: .bodyPartFileSizeQueryFailedWithError(url:fileURL))
            return
        }
        ...
        guard let stream = InputStream(url: fileURL) else {
            setBodyPartError(withReason: .bodyPartInputStreamCreationFailed(url:fileURL))
            return
        }
    ...
    public func writeEncodedData(to fileURL: URL) throws {
        ...
        if fileManager.fileExists(atPath: fileURL.path) {
            throw CSError.multipartEncodingFailed(reason: .outputStreamFileAlreadyExists).error(msg: "\(fileURL)")
        } else if !fileURL.isFileURL {
            throw CSError.multipartEncodingFailed(reason: .outputStreamURLInvalid).error(msg: "\(fileURL)")
        }

        guard let outputStream = OutputStream(url: fileURL, append: false) else {
            throw CSError.multipartEncodingFailed(reason: .outputStreamCreationFailed).error(msg: "\(fileURL)")
        }
    ...
    private func encodeBodyStream(for bodyPart: BodyPart) throws -> Data {
        ...
        while inputStream.hasBytesAvailable {
            ...
            if let error = inputStream.streamError {
                throw CSError.multipartEncodingFailed(reason: .inputStreamReadFailed).error(msg: "\(error)")
            }
    ...
    private func write(_ buffer: inout [UInt8], to outputStream: OutputStream) throws {
        ...
        while bytesToWrite > 0, outputStream.hasSpaceAvailable {
            ...
            if let error = outputStream.streamError {
                throw CSError.multipartEncodingFailed(reason: .outputStreamWriteFailed).error(msg: "\(error)")
            }
    ...
    private func setBodyPartError(withReason reason: CSError.MultipartEncodingFailureReason) {
        guard bodyPartError == nil else { return }
        bodyPartError = CSError.multipartEncodingFailed(reason: reason)
    }
}
